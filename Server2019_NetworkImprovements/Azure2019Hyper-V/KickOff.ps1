...
$Attempt = 'o'
$purpose = '2019NetworkingDemo' # removed the etag

$TemplateFile = "$pwd\azuredeploy.json"
$Location = 'East US'
$rgname = "HyperV$Attempt`-rg" # working on better OUs & Autoshutdown
# $saname = "genesyssa$Attempt"     # Lowercase required
$addnsName = "genesysad$Attempt"     # Lowercase required
$adVMName = 'HyperVHost01-vm'
[string] $Shutdowntime = (get-date).AddHours(1).hour * 100  # Have the VM shutdown in 3 hours.
$KeyVaultName = "Vault-$($rgname.replace('.',''))-$(get-random -min 1000 -max 9999)-kv"  # Must match pattern '^[a-zA-Z0-9-]{3,24}$'
$Cred = [System.Management.Automation.PSCredential]::new('Gene', $('Password!101' | ConvertTo-SecureString -AsPlainText -Force))
...
if (-not (Get-AzureRmResourceGroup -Name $rgname -ErrorAction 'SilentlyContinue'))
{
...
# Create a Key Vault
$ExistingKeyVaults = get-azurermkeyvault |? {$_.ResourceGroupName -eq $rgname}
if ($ExistingKeyVaults)
{
    $ExistingKeyVaults | Remove-AzureRmKeyVault -Force
}
...
Get-ChildItem .\HyperVSetup.ps* | copy-item -ToSession $session -Destination c:\DSC\
# SetupScripts
...
invoke-command { install-module -Name Pester -Repository PSGallery -RequiredVersion '4.8.1' -AllowClobber -Force -SkipPublisherCheck } -session $Session
# Write-Verbose "[$(Get-Date -format G)]  - xActiveDirectory"
# invoke-command {install-module -Name xActiveDirectory -Repository PSGallery -RequiredVersion '2.26.0.0' -AllowClobber -Force } -session $Session
...
Write-Verbose "[$(Get-Date -format G)]  - xHyper-V"
invoke-command { install-module -Name xHyper-V -Repository PSGallery -RequiredVersion '3.16.0.0' -AllowClobber -Force } -session $Session
...
invoke-command  -Session $session {& c:\dsc\HyperVSetup.ps1} 
# invoke-command  -Session $session {HyperVSetup -OutputPath 'c:\DSC\HyperVSetup'} 
...
invoke-command {Set-DscLocalConfigurationManager 'c:\dsc\HyperVSetup\' -Verbose} -session $session

# Start the DSC configuration
Write-Verbose "[$(Get-Date -format G)] Start DSC Configuration"
invoke-command {Start-DscConfiguration -Path 'c:\dsc\HyperVSetup\' -Wait -Verbose} -session $session
